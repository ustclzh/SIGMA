...
class Posterior_Density_Inference:
    def __init__(self, SDE_Model, negllik, u, theta_SDE, theta_GP_mu, theta_GP_sigma, logsigmasq, u_KL_shape, theta_SDE_shape, theta_GP_mu_shape, theta_GP_sigma_shape, sigma_shape, ind_non_obs, noisy_known=False, lsteps=50, epsilon=1e-5, n_samples=4000, upper_bound = None, lower_bound = None, burn_in_ratio = 0.1):
        all_theta=self.vectorize(u, theta_SDE, theta_GP_mu, theta_GP_sigma, logsigmasq)
        print(all_theta.shape)
        self.ind_non_obs = ind_non_obs
        self.SDE_Model = SDE_Model
        self.all_theta = all_theta
        self.theta_SDE_shape = theta_SDE_shape
        self.theta_GP_mu_shape = theta_GP_mu_shape
        self.theta_GP_sigma_shape = theta_GP_sigma_shape
    ...
    def vectorize(self, u, theta_SDE, theta_GP_mu, theta_GP_sigma, logsigmasq=None):
        t1 = torch.reshape(u.detach(), (-1,))
        t2 = torch.reshape(theta_SDE.detach(), (-1,))
        t3 = torch.reshape(theta_GP_mu.detach(), (-1,))
        t4 = torch.reshape(theta_GP_sigma.detach(), (-1,))

        if logsigmasq is not None:
            t5 = torch.reshape(logsigmasq.detach(), (-1,))
        else:
            t5=torch.tensor(torch.ones(0))
        long_vec = torch.cat((t1, t2, t3, t4))
        return long_vec
    ...
    def devectorize(self, long_tensor, u_shape, theta_SDE_shape, theta_GP_mu_shape, theta_GP_sigma_shape, sigma_shape = None):
        u_dim = self.get_dim(u_shape)
        theta_SDE_dim = self.get_dim(theta_SDE_shape)
        theta_GP_mu_dim = self.get_dim(theta_GP_mu_shape)
        theta_GP_sigma_dim = self.get_dim(theta_GP_sigma_shape)
        ...
        theta_GP_mu = torch.reshape(long_tensor[u_dim + theta_SDE_dim : u_dim + theta_SDE_dim + theta_GP_mu_dim],theta_GP_mu_shape)
        theta_GP_sigma = torch.reshape(long_tensor[u_dim + theta_SDE_dim + theta_GP_mu_dim : u_dim + theta_SDE_dim + theta_GP_mu_dim + theta_GP_sigma_dim],theta_GP_sigma_shape)
        #if sigma_shape is not None: logsigmasq = torch.reshape(long_tensor[u_dim + theta_SDE_dim + theta_GP_mu_dim + theta_GP_sigma_dim : u_dim + theta_SDE_dim + theta_GP_mu_dim + theta_GP_sigma_dim + sigma_dim],sigma_shape)
        #else: logsigmasq=torch.ones(0)
        return xlatent, theta_SDE, theta_GP_mu, theta_GP_sigma

    def Minus_Log_Posterior_vec(self, all_theta):
        xlatent_0, theta_SDE, theta_GP_mu, theta_GP_sigma = self.devectorize(all_theta, self.u_KL_shape, self.theta_SDE_shape, self.theta_GP_mu_shape, self.theta_GP_sigma_shape, self.sigma_shape)
        return self.Minus_Log_Posterior(xlatent_0, theta_SDE, theta_GP_mu, theta_GP_sigma)

    def Nabla(self, theta_torch):
        theta_torch = theta_torch.detach()
        xlatent, theta_SDE, theta_GP_mu, theta_GP_sigma= self.devectorize(theta_torch, self.u_KL_shape, self.theta_SDE_shape, self.theta_GP_mu_shape, self.theta_GP_sigma_shape, self.sigma_shape)
        xlatent.requires_grad = True  
        theta_SDE.requires_grad = True
        theta_GP_mu.requires_grad = True
        theta_GP_sigma.requires_grad = True
        llik = self.Minus_Log_Posterior(xlatent, theta_SDE, theta_GP_mu, theta_GP_sigma)
        llik.backward() 
        #print(xlatent.grad, theta_SDE.grad, theta_GP_mu.grad, theta_GP_sigma.grad)
        v = self.vectorize(xlatent.grad, theta_SDE.grad, theta_GP_mu.grad, theta_GP_sigma.grad)
    ...
    def Sampling(self):
        ...
        samples_theta_gp_u = torch.zeros((self.total_samples, self.theta_GP_mu_shape[0]))
        if len(self.theta_GP_mu_shape) == 2: samples_theta_gp_u = torch.zeros((self.total_samples, self.theta_GP_mu_shape[0], self.theta_GP_mu_shape[1]))
        samples_theta_gp_sigma = torch.zeros((self.total_samples, self.theta_GP_sigma_shape[0]))
        ...
        for EachIter in range(self.total_samples):
            ...
            samples_u[EachIter], samples_theta_sde[EachIter], samples_theta_gp_u[EachIter], samples_theta_gp_sigma[EachIter] = self.devectorize(cur_theta.clone(), self.u_KL_shape, self.theta_SDE_shape, self.theta_GP_mu_shape, self.theta_GP_sigma_shape, self.sigma_shape)
        ...
        u_mean, theta_SDE, theta_GP_mu, theta_GP_sigma = self.devectorize(torch.tensor(post_mean_all), self.u_KL_shape, self.theta_SDE_shape, self.theta_GP_mu_shape, self.theta_GP_sigma_shape, self.sigma_shape)
        HMC_sample = {
            'samples': samples,
            ...
            'posterior_mean_theta_GP_mu' : theta_GP_mu,
            'posterior_mean_theta_GP_sigma' : theta_GP_sigma,
